...
def test_get_scoped_token_uses_native_when_flag_enabled(monkeypatch):
    ...
    with (
        mock.patch.object(
            repo, "_get_scoped_token_native", return_value=sentinel
        ) as native,
...
def test_get_scoped_token_uses_legacy_when_flag_disabled(monkeypatch):
    ...
    with (
        mock.patch.object(
            repo, "_get_scoped_token_legacy", return_value=sentinel
        ) as legacy,
        mock.patch.object(repo, "_get_scoped_token_native") as native,
    ):
        result = repo._get_scoped_token(lineage_header_info=None)

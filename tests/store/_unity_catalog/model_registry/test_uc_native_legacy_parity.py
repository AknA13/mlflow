"""Shadow-parity tests for the UC-native model-registry client migration.

These assert that the migrated client, when it reads a model / model version off the
**native** governance surface (``/api/2.1/unity-catalog/*``, ``UcRegisteredModelInfo`` /
``UcModelVersionInfo`` with the MLflow-enrichment siblings the UCMR enriched handlers add),
produces an MLflow entity that is **field-for-field identical** to the one the pre-migration
client produced from the **legacy** MLflow-dialect surface (``/mlflow/unity-catalog/*``,
``RegisteredModel`` / ``ModelVersion``).

The legacy converters are pinned inline (copied from the pre-migration
``mlflow/utils/_unity_catalog_utils.py``) so the parity check is a stable oracle and does not
depend on the legacy code still existing in the tree. This is the client-side counterpart of the
server's ``UcMiucExternalizationIntegrationSuite`` -- the server proves the native handler emits
the governance entity byte-identically plus enrichment siblings; this proves the client folds
that native shape back into the same MLflow entity the legacy shape produced.
"""
...
from mlflow.protos.databricks_uc_registry_messages_pb2 import (
    ModelVersionDeploymentJobState as ProtoMVDeploymentJobState,
)
from mlflow.protos.databricks_uc_registry_messages_pb2 import (
...
def _native_mv_deployment_job_state():
    return ProtoMVDeploymentJobState(
        job_id="42",
        run_id="7",
...
def test_model_version_native_legacy_parity():
    full_name = "catalog.schema.model"
    dep_state = _native_mv_deployment_job_state()
    legacy = LegacyModelVersion(
        ...
        model_id="logged-1",
        deployment_job_state=dep_state,
    )
    native = UcModelVersionInfo(
        model_name="model",
        catalog_name="catalog",
        schema_name="schema",
        version=4,
